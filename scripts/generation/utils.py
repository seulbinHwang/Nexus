...
from utils import NpAgentState, NpEgoState
...
FEATURE_STD = [52.0, 52.0, 0.5, 0.5, 2.0, 2.0, 2.5, 0.8]
...
def get_global_to_local_transform(pose: StateSE2):
...
def invert_dict(input_dict):
    ...
    return {k:{vv:kk for kk,vv in v.items()} for k, v in input_dict.items()}
...
def decode_scene_tensor(
    ...
    if isinstance(scene_tensor, torch.Tensor):
        return scene_tensor * 2.0 * torch.tensor(
            FEATURE_STD, device=scene_tensor.device
        ) + torch.tensor(FEATURE_MEANS, device=scene_tensor.device)
    else:
        return scene_tensor * 2.0 * np.array(FEATURE_STD) + np.array(FEATURE_MEANS)

def scene_tensor_to_dict(scene_tensor, idx2token):
    """Convert a scene tensor to a dictionary."""
    if isinstance(scene_tensor, torch.Tensor):
        scene_tensor = scene_tensor.cpu().detach().numpy()
    scene_dict = {}
    # scene_tensor: (n_max_agents, n_timesteps, N_SCENE_TENSOR_FEATURES)
    # idx2token: {idx: track_token}
    for idx, track_token in idx2token.items():
        agent = scene_tensor[idx]
