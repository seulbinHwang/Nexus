...
from nuplan_extent.planning.scenario_builder.wod_db.wod_scenario_v1_1 import \
    WodScenario
...
class WodScenarioBuilder(AbstractScenarioBuilder):
    ...
    def get_scenarios(self, scenario_filter: ScenarioFilter,
        ...
        #         self.data_root, 'training_token_list.txt')
        # if self.validation_token_list_path is None:
        #     self.validation_token_list_path = os.path.join(
        #         self.data_root, 'validation_token_list.txt')

        if self.training_token_list_path is not None:  
        ...
        if self.validation_token_list_path is not None:        
        ...
        assert len(scenarios) > 0, "No scenarios found in the database."
