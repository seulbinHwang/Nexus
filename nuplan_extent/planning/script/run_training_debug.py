...
import sys
import os
# nuplan map path and split path
sys.path.append('/cpfs01/shared/opendrivelab/opendrivelab_hdd/nuplan/dataset/maps')
sys.path.append('/cpfs01/shared/opendrivelab/opendrivelab_hdd/nuplan/dataset/nuplan-v1.1/splits/trainval')
sys.path.append('/cpfs01/user/yenaisheng/SceneGen/third_party/nuplan-devkit')
# working path
sys.path.append('/cpfs01/user/yenaisheng/SceneGen')
# wandb entity
os.environ['WANDB_ENTITY'] ='opendrivelab'
os.environ['WANDB_PROJECT']='scenegen'
os.environ['DEBUG']='True'
# os.environ["WANDB_EXP_NAME"]='test'
...
def main(cfg: DictConfig) -> Optional[TrainingEngine]:
    ...
    :param cfg: omegaconf dictionary
    """
    # cfg.lightning.trainer.params.default_root_dir=cfg.output_dir
    # print('log_dir:', cfg.lightning.trainer.params.default_root_dir)
    # Fix random seed
    ...
    # save the cfg
    # with open('/cpfs01/user/yenaisheng/SceneGen/nuplan_extent/planning/script/config/cfg_cache_demo.yaml', 'w') as f:
    #     OmegaConf.save(cfg, f)
    # print('cfg saved at /cpfs01/user/yenaisheng/SceneGen/nuplan_extent/planning/script/config/cfg_cache_demo.yaml')
    # return 
    ...
    if cfg.py_func == "train":
        ...
        if os.environ.get("USE_WANDB", None):
            # check if wandb is enabled
            if True and int(os.environ.get("LOCAL_RANK", 0)) == 0:
        ...
        if hasattr(cfg, "checkpoint") and cfg.checkpoint.get("ckpt_path") is not None:
            engine.trainer.fit(
                model=engine.model,
                datamodule=engine.datamodule,
                ckpt_path=cfg.checkpoint.get("ckpt_path"),
            )
        else:
...
if __name__ == "__main__":
    # /cpfs01/user/yenaisheng/waymo_test_config.yaml
    # /cpfs01/user/yenaisheng/nuplan_train_config.yaml
    with open('/cpfs01/user/yenaisheng/SceneGen/nuplan_extent/planning/script/config/cfg_wod_train.yaml', 'r') as f:
        cfg = OmegaConf.load(f)
    # with open('/cpfs01/user/yenaisheng/SceneGen/nuplan_extent/planning/script/config/cfg_cache_demo.yaml', 'r') as f:
    #     cfg = OmegaConf.load(f)
    # change the time
    # from datetime import datetime
    # current_time = datetime.now().strftime("%Y.%m.%d.%H.%M.%S")
    # original_time = cfg['experiment_uid']
    # cfg['experiment_uid'] = cfg['experiment_uid'].replace(original_time, current_time)
    # cfg['output_dir'] = cfg['output_dir'].replace(original_time, current_time)
    # cfg['callbacks']['model_checkpoint_callback']['dirpath'] = cfg['callbacks']['model_checkpoint_callback']['dirpath'].replace(original_time, current_time)
    # cfg['experiment'] = cfg['experiment'].replace(original_time, current_time)
    main(cfg)
