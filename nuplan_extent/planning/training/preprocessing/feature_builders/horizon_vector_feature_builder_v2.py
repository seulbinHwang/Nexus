...
from nuplan_extent.planning.training.preprocessing.features.vector_builders import get_global_to_local_transform, get_local_to_global_transform
...
class HorizonVectorV2(AbstractModelFeature):
    ...
    def unpack(self) -> List[AbstractModelFeature]:
        ...
        return [
            HorizonVectorV2(data={name: self.data[name][i] for name in self.data})
            for i in range(batch_size)
        ]


class HorizonVectorFeatureBuilderV2(
    AbstractFeatureBuilder, PreparedScenarioFeatureBuilder
):
    """
    ...
    def get_features_from_scenario(
        ...
        vectors['offset'] = prepared_scenario.get_offset()
        vectors['center'] = self._builders['agents'].calc_raster_center(ego_state)
        vectors['trans'] = get_local_to_global_transform(vectors['center'])
        vectors['lidarpc'] = prepared_scenario._data['lidarpc']
        vectors['track_token_id_mapping'] = prepared_scenario._data['track_token_id_mapping']
        vectors['map_name'] = scenario.map_api.map_name
        vectors['log_name'] = scenario.log_name
        vectors['scenario_token'] = scenario.token
