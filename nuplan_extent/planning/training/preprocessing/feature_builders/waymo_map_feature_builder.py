...
from third_party.MTR.mtr.datasets.waymo.waymo_types import lane_type, road_line_type, road_edge_type, polyline_type
from third_party.MTR.mtr.utils import common_utils
...
class WaymoMapFeatureBuilder(AbstractFeatureBuilder):
    ...
    def __init__(self, max_num_map_objects: int, num_points_each_polyline: int, radius: float, center_offset: List[float]) -> None:
        super().__init__()
        ...
        self._num_points_each_polyline = num_points_each_polyline
    ...
    def get_features_from_scenario(
        ...
        for map_layer, map_objects in near_map_objects.items():
            if map_layer == SemanticMapLayer.LANE: # center_line of lane 
            ...
            elif map_layer == SemanticMapLayer.BASELINE_PATHS: # road_line (e.g. double yellow line, ...)
                for map_object in map_objects:
            ...
            elif map_layer == SemanticMapLayer.EXTENDED_PUDO: # road_edge (e.g. median, boundary, ...)
                for map_object in map_objects:
            ...
            elif map_layer == SemanticMapLayer.STOP_SIGN: # stop point of stop sign (one point with x,y,z)
                for map_object in map_objects:
            ...
            elif map_layer == SemanticMapLayer.CROSSWALK: # crosswalk (polygon shape)
                for map_object in map_objects:
            ...
            elif map_layer == SemanticMapLayer.SPEED_BUMP: # speed bump (polygon shape)
                for map_object in map_objects:
        ...
        try:
            ...
            map_polylines = np.zeros((1,self._num_points_each_polyline,9))  # (1, num_points_each_polyline, 7)
            map_polylines_mask = np.zeros((1,self._num_points_each_polyline))  # (1, num_points_each_polyline)        
            map_polylines_center = np.zeros((1,3))  # (1, 3)
            ret_dict = {}
