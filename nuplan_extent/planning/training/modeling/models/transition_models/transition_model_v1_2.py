...
class TransitionModelV1_2(nn.Module):
    def __init__(self,
    ...
    def forward_train(
        ...
        token_embeddings, frame_index = embedder.get_token_embeddings_and_time_index(
            tokenized_arrays, latent_features, image_features.dtype, image_features.device, dynamic_forward=False) # True
        output_features = self.transition_model.forward(
            token_embeddings, image_features)
        # output_features = token_embeddings
        # print(self.diffusion_model.parameters())
        assert token_embeddings.shape[1] == frame_index.shape[1]
        # input()
        diffusion_out = None
        # if self.training:
        diffusion_out = self.diffusion_model.forward(output_features, frame_index)
        ...
        # arr = pred_agent_tokens - target_tokenized_state
        # arr = torch.abs(arr.to(float))
        # print(torch.mean(arr))
        # import pdb; pdb.set_trace()
        
        return pred_agent_logits, pred_agent_tokens, target_tokenized_state, pred_control_logits, target_ctrl_tokens, hidden, diffusion_out
    ...
    def forward_inference_without_cache(
        ...
        for i in range(num_imagine_frames):
            ...
            last_tokenized_arrays = gutils.add_one_to_frame_index(last_tokenized_arrays)
            # gutils.analysis(last_tokenized_arrays)
                # save imagined frame to history
            ...
            pred_agent_logits, pred_agent_tokens, target, _ = token_decoder.decoding_agents(
                embedder, output_features, input_tokenized_arrays, last_frame_only=True)
            # update last frame with predicted agent tokens
            pred_list.append(pred_agent_tokens)
        ...
        # pred_agent_tokens = torch.vstack(pred_list)
        # target = torch.vstack(target_list)
        # arr = pred_agent_tokens - target
        # arr = torch.abs(arr.to(float))
        # print(torch.mean(arr), 'infra')
        # import pdb; pdb.set_trace()

        return hist_tokenized_arrays


    def forward_inference_without_cache_v1(
                self,
                image_features,
                tokenized_arrays,
                embedder,
        ...
        tokenized_arrays = self._forward_inference_without_cache(image_features, tokenized_arrays, embedder, token_decoder, render, latent_features, num_imagine_frames, num_conditioned_frames, update_initial_prompts)
        

        token_embeddings, frame_index = embedder.get_token_embeddings_and_time_index(
            tokenized_arrays, latent_features, image_features.dtype, image_features.device, dynamic_forward=True) # True
        # output_features = self.diffusion_model.inference(token_embeddings, frame_index, num_conditioned_frames)
        output_features = token_embeddings
        ...
        arr = hist_tokenized_arrays-tokenized_arrays
        print(arr)
        import pdb; pdb.set_trace()
    ...
    def forward_inference_without_cache_v(
                self,
                image_features,
                tokenized_arrays,
                embedder,
        ...
        # tokenized_arrays = self._forward_inference_without_cache(image_features, tokenized_arrays, embedder, token_decoder, render, latent_features, num_imagine_frames, num_conditioned_frames, update_initial_prompts)
        # last_tokenized_arrays = deepcopy(tokenized_arrays)
        # gutils.analysis(tokenized_arrays)
        # hist_tokenized_arrays = gutils.extract_history_data(
        #         deepcopy(tokenized_arrays), num_conditioned_frames)
        raw_array = deepcopy(tokenized_arrays)

        hist_tokenized_arrays = np.full(tokenized_arrays.shape, np.nan)
        last_tokenized_arrays = gutils.extract_sceanrio_data(deepcopy(tokenized_arrays), mask_future=False)

        # tokenized_arrays = deepcopy(last_tokenized_arrays)
        token_embeddings, frame_index = embedder.get_token_embeddings_and_time_index(
            last_tokenized_arrays, latent_features, image_features.dtype, image_features.device, dynamic_forward=True) # True
        assert token_embeddings.shape[1] == frame_index.shape[1]

        token_embeddings = self.transition_model.forward(token_embeddings, image_features, enable_cache=self.enable_qkv_cache, visual_cache=True)
        # output_features = self.diffusion_model.inference(token_embeddings, frame_index, num_conditioned_frames)
        # output_features = token_embeddings
        # import pdb; pdb.set_trace()

        diffusion_out = self.diffusion_model.forward(token_embeddings, frame_index)
        output_features = diffusion_out['xs_pred']

        # output_features = gutils.get_frame_data(output_features, frame_index)

        pred_agent_logits, pred_agent_tokens, target, _, return_list = token_decoder.decoding_agents_single_frame(
            embedder, output_features, last_tokenized_arrays, last_frame_only=False)
        ...
        # gutils.analysis(last_tokenized_arrays)
        # print(pred_agent_tokens.shape)
        
        last_tokenized_arrays = render.update_last_frame_data(
                last_tokenized_arrays, pred_agent_tokens.cpu().detach().numpy(), return_list)
        last_tokenized_arrays = render(last_tokenized_arrays, hist_tokenized_arrays)
