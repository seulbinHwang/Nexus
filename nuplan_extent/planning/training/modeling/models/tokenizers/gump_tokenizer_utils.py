...
def pad_array(arr):
    ...
    Returns:
        np.ndarray: A numpy array representing the BOS token with the frame index.
    """
    bos_array = np.zeros((19,)) - 1
    bos_array[:arr.shape[0]] = arr
    return bos_array
...
@jit(nopython=True)
def reorgnize_array(tokenized_arrays, T=21, block_size=1024):
    B, max_len, dim = tokenized_arrays.shape

    orgnized_arrays = np.full((B, T, block_size, dim), np.nan)
    mask = np.zeros((B, T, block_size))
    max_index = 0
    ...
    for i in range(B): 
        ...
        for j in range(max_len):
            ...
            if current_token[TOKEN_TYPE_IDX] in [TokenType.EGO_TOKEN.value, TokenType.NEWBORN_AGENT_TOKEN.value, TokenType.AGENT_TOKEN.value]:
                time = int(current_token[FRAME_INDEX_IDX])
                index = int(current_token[TRACK_ID_DIM])
                max_index = max(index, max_index)
                if index < block_size:
                    orgnized_arrays[i,time,index,:] = current_token
                    mask[i,time,index] = 1
    return orgnized_arrays, mask

@jit(nopython=True)
def reformat_array(last_tokenized_arrays, tokenized_arrays,T=21):
    new_pad_tokenized_arrays = np.full(tokenized_arrays.shape, np.nan)

    for i in range(last_tokenized_arrays.shape[0]):
        index = 0
        for j in range(last_tokenized_arrays.shape[1]):
            current_token = last_tokenized_arrays[i,j]
            if np.isnan(current_token).any():
                continue

            frame_id = current_token[FRAME_INDEX_IDX]
            track_id = current_token[TRACK_ID_DIM]

            if current_token[TOKEN_TYPE_IDX] in [TokenType.EGO_TOKEN.value]:
                new_pad_tokenized_arrays[i,index] = pad_array(get_bos_array(frame_id))
                index = safe_add_one(index, tokenized_arrays.shape[1])
                new_pad_tokenized_arrays[i,index] = current_token
                index = safe_add_one(index, tokenized_arrays.shape[1])
            if current_token[TOKEN_TYPE_IDX] in [TokenType.NEWBORN_AGENT_TOKEN.value, TokenType.AGENT_TOKEN.value]:
                new_pad_tokenized_arrays[i,index] = current_token
                index = safe_add_one(index, tokenized_arrays.shape[1])

    return new_pad_tokenized_arrays

@jit(nopython=True)
def get_feature_data(tokenized_arrays):

    state_index = [int(X_DIM), int(Y_DIM), int(HEADING_DIM), int(WIDTH_DIM), int(LENGTH_DIM), int(VX_DIM), int(VY_DIM)]
    tokenized_index = [int(TOKENIZED_X_DIM), int(TOKENIZED_Y_DIM), int(TOKENIZED_HEADING_DIM)]

    state_embedding_features = np.zeros((tokenized_arrays.shape[0], tokenized_arrays.shape[1], tokenized_arrays.shape[2], 7), dtype=np.float64)
    tokenized_embedding_features = np.zeros((tokenized_arrays.shape[0], tokenized_arrays.shape[1], tokenized_arrays.shape[2], 3), dtype=np.int64)

    for i in range(tokenized_arrays.shape[0]):
        for j in range(tokenized_arrays.shape[1]):
            for k in range(tokenized_arrays.shape[2]):
                current_token = tokenized_arrays[i,j,k]
                if np.isnan(current_token).any():
                    continue
                ...
                tx, ty, tz = int(current_token[TOKENIZED_X_DIM]), int(current_token[TOKENIZED_Y_DIM]), int(current_token[TOKENIZED_HEADING_DIM])

                state_embedding_features[i,j,k] = np.array([x, y, heading, w, l, vx, vy])
                tokenized_embedding_features[i,j,k] = np.array([tx, ty, tz])

    return state_embedding_features, tokenized_embedding_features
...
@jit(nopython=True)
def get_frame_tuple(tokenized_arrays, block_size=1024, max_frames=1000):

    batch_size, T, seq_len, _ = tokenized_arrays.shape
    frame_indices = (max_frames-1) * np.ones((batch_size, T, block_size,), dtype=np.int64)
    for i in range(tokenized_arrays.shape[0]):
        for j in range(tokenized_arrays.shape[1]):
            for m in range(tokenized_arrays.shape[2]):
                current_token = tokenized_arrays[i, j, m]
                if np.isnan(current_token).any():
                    continue
                frame_index = current_token[FRAME_INDEX_IDX]
                frame_indices[i, j, m] = frame_index
                
    return frame_indices


@jit(nopython=True)
def crop_array_by_mask(pred_agent_logits, pred_agent_tokens, target_tokenized_state, mask):
    pred_agent_logits = rearrange(pred_agent_logits, "B T seq dim log -> (B T seq) dim log")
    pred_agent_tokens = rearrange(pred_agent_tokens, "B T seq dim -> (B T seq) dim")
    target_tokenized_state = rearrange(target_tokenized_state, "B T seq dim -> (B T seq) dim")
    mask = rearrange(mask, "B T seq -> (B T seq)")
    index = mask==1
    return pred_agent_logits[index], pred_agent_tokens[index], target_tokenized_state[index]
...
# @jit(nopython=True)
def get_agent_target_inds(tokenized_arrays, block_size=1024, last_frame_only=False, return_list=False):
    """
    ...
    for i in range(tokenized_arrays.shape[0]):
        ...
        for j in range(tokenized_arrays.shape[1]):
            ...
            if token_type in [TokenType.BOS_TOKEN.value, TokenType.NEWBORN_BOS_TOKEN.value, TokenType.PAD_TOKEN.value]:
                ...
                inbatch_list.append(embed_seq_index)
            elif token_type in [TokenType.EGO_TOKEN.value, TokenType.AGENT_TOKEN.value, TokenType.NEWBORN_AGENT_TOKEN.value]:
                track_id = current_token[TRACK_ID_DIM]
                class_index = current_token[CLASS_TYPE_DIM]
                ...
                print('invalid type:', token_type)
        index_list.append(inbatch_list)
    query_embedding_inds = query_embedding_inds[:query_index]
    state_embedding_features = state_embedding_features[:state_index]
    if return_list:
        return query_embedding_inds, state_embedding_features, index_list
...
@jit(nopython=True)
def extract_sceanrio_data(tokenized_arrays, skip_nb_bos=True, mask_future=True):
    """
    Extracts the last frame data from the given tokenized arrays.
    ...
    Notes:
        ...
        - The last frame index is incremented by 1 for each token in the last frame.
    """
    hist_tokenized_arrays = tokenized_arrays
    last_frame_arrays = np.nan * np.ones_like(hist_tokenized_arrays)
    for i in range(hist_tokenized_arrays.shape[0]):
        last_frame_seq_index = 0
        for j in range(hist_tokenized_arrays.shape[1]):
            ...
            if skip_nb_bos and current_token[TOKEN_TYPE_IDX] in [TokenType.NEWBORN_AGENT_TOKEN.value]:
                current_token[TOKEN_TYPE_IDX] = TokenType.AGENT_TOKEN.value
            if current_token[FRAME_INDEX_IDX] >= 4 and mask_future:
                current_token[TOKENIZED_X_DIM] = 1000
                current_token[TOKENIZED_Y_DIM] = 2000
                current_token[TOKENIZED_HEADING_DIM] = 2500
            last_frame_arrays[i, last_frame_seq_index] = current_token
            last_frame_seq_index = safe_add_one(last_frame_seq_index, last_frame_arrays.shape[1])
    return last_frame_arrays
...
# @jit(nopython=True)
def update_last_frame_data(last_tokenized_arrays, pred_agent_tokens, return_list=None, block_size=2048):
    """
    ...
    for i in range(len(last_tokenized_arrays)): 
        ...
        for j in range(len(last_tokenized_arrays[i])):
            ...
            if np.isnan(current_token).any():
                break
            if current_token[TOKEN_TYPE_IDX] in [TokenType.BOS_TOKEN.value]:
                max_token += 1
            if current_token[TOKEN_TYPE_IDX] in [TokenType.EGO_TOKEN.value, TokenType.NEWBORN_AGENT_TOKEN.value, TokenType.AGENT_TOKEN.value]:  
                ...
                if return_list is not None:
                    if max_token >= len(return_list[i]):
                        break
...
# @jit(nopython=True)
def update_frame_data(last_tokenized_arrays, pred_agent_tokens, return_list=None, block_size=2048):
    """
    update last frame data with predicted agent tokens
    :param last_tokenized_arrays: np.ndarray, shape (batch_size, max_seq_len, NpTokenizedSequence_DIM)
    :param pred_agent_tokens: np.ndarray, shape (num_agents, [x, y, heading] tokens)
    ...
    for i in range(len(last_tokenized_arrays)): 
        for j in range(len(last_tokenized_arrays[i])):
            ...
            if np.isnan(current_token).any():
                continue
            last_tokenized_arrays[i, j, TOKENIZED_X_DIM] = pred_agent_tokens[i, j, 0]
            last_tokenized_arrays[i, j, TOKENIZED_Y_DIM] = pred_agent_tokens[i, j, 1]
            last_tokenized_arrays[i, j, TOKENIZED_HEADING_DIM] = pred_agent_tokens[i, j, 2]

    return last_tokenized_arrays

@jit(nopython=True)
def analysis(last_tokenized_arrays):
    stat_dict = []
    num_dict = []
    for i in range(len(last_tokenized_arrays)): 
        num = 0
        for j in range(len(last_tokenized_arrays[i])):
            ...
            if np.isnan(current_token).any():
                break
            ele_type = current_token[TOKEN_TYPE_IDX]
            if num >= 2048:
                break
            if ele_type in [TokenType.EGO_TOKEN.value]:
                stat_dict.append('EGO_TOKEN')
                num += 2
            elif ele_type in [TokenType.NEWBORN_AGENT_TOKEN.value]:
                stat_dict.append('NEWBORN_AGENT_TOKEN')
                num += 2
            elif ele_type in [TokenType.AGENT_TOKEN.value]:
                stat_dict.append('AGENT_TOKEN')
                num += 2
            elif ele_type in [TokenType.BOS_TOKEN.value]:
                stat_dict.append('BOS_TOKEN')
                num += 1
            else:
                stat_dict.append('other')
                num += 1
        num_dict.append(num)
    print(num_dict)
    element_count = {x: stat_dict.count(x) for x in stat_dict}
    print(len(stat_dict), element_count)

@jit(nopython=True)
def get_frame_data(output_features, frame_index, frame_id=0):
    assert output_features.shape[1] == frame_index.shape[1]
    output = []
    for i in range(output_features.shape[0]):
        current_feature = output_features[i]
        out_feature = torch.zeros(current_feature.shape)

        first_pos = torch.where(frame_index[i] == frame_id)[0][0].item()
        last_pos = torch.where(frame_index[i] == frame_id)[0][-1].item()

        out_feature[:(last_pos-first_pos)] = current_feature[first_pos:last_pos]
        output.append(torch.unsqueeze(out_feature, dim=0))
