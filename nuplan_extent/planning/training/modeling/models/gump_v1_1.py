...
import time
from nuplan_extent.planning.training.preprocessing.features.scene_tensor import SceneTensor, FEATURE_MEANS, FEATURE_STD
from nuplan_extent.planning.training.modeling.models.tokenizers.gump_tokenizer_utils import X_DIM,Y_DIM,HEADING_DIM,VX_DIM,VY_DIM,LENGTH_DIM,WIDTH_DIM,reorgnize_array
...
class GUMPV1_1(TorchModuleWrapper):
    ...
    def forward_inference(self, input_features: FeaturesType, n_repeat: int = 1, num_imagine_frames: int = 1) -> Dict:
        ...
        # Encode image
        image_features = self.image_encoder(input_features)
        # just repeat once for fair comparison in time consumption
        n_repeat = 1
        # Tokenize vectors
        ...
        # add in out: valid_mask, scene_tensor_gt, behavior_pred, time_consume
        # to compitible with the aggregated metrics of nuplan metrics
        out = {"time_consume": time_consume}
        out['scene_tensor_gt'], out['valid_mask'] = reorgnize_array(tokenized_arrays[:,0], T=21, block_size=256)
        out['behavior_pred'], mask = reorgnize_array(predicted_tokenized_arrays[:,0], T=21, block_size=256)
        out['valid_mask'] = out['valid_mask'] * mask
        out['valid_mask'] = rearrange(out['valid_mask'], 'b t n -> b n t')
        out['valid_mask'] = torch.tensor(out['valid_mask'], dtype=torch.float32, device=pred_agent_logits.device)
        out["road_graph"] = input_features['raster'].data["road_graph"]
        out["road_graph_validity"] = input_features['raster'].data["road_graph_validity"]
        # import pdb; pdb.set_trace()
        for name in ['scene_tensor_gt', 'behavior_pred']:
            out[name] = out[name][...,[X_DIM,Y_DIM,HEADING_DIM,VX_DIM,VY_DIM,LENGTH_DIM,WIDTH_DIM]]
            heading = out[name][..., 2]
            out[name] = np.concatenate((out[name][..., :2], np.cos(heading)[..., np.newaxis], np.sin(heading)[..., np.newaxis], out[name][..., 3:]), axis=-1)
            out[name] = (out[name] - np.array(FEATURE_MEANS)) / np.array(FEATURE_STD) / 2.0
            out[name] = np.nan_to_num(out[name],nan=0.0)
            out[name] = torch.tensor(out[name], dtype=torch.float32, device=pred_agent_logits.device)
            out[name] = rearrange(out[name], 'b t n c -> b n t c')            
        # import pdb; pdb.set_trace()
        ...
        return {
            ...
            'predicted_tokenized_arrays': predicted_tokenized_arrays,
            'gt_tokenized_arrays': gt_tokenized_arrays,
            "behavior_prediction_out": out,
            # "road_graph": input_features['raster'].data["road_graph"],
            # "road_graph_validity": input_features['raster'].data["road_graph_validity"] 
        }
