...
class ResnetBlock(nn.Module):
    def __init__(
        ...
        self.in_layers = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Conv3d(dim, dim_out, kernel_size=(1, 1, 1), padding=(0, 0, 0)),
        )

        self.out_layers = nn.Sequential(
            nn.GroupNorm(num_groups=groups, num_channels=dim_out, eps=eps),
            nn.SiLU(),
            nn.Conv3d(dim_out, dim_out, kernel_size=(1, 1, 1), padding=(0, 0, 0)),
        )
    ...
    def forward(self, x: torch.Tensor, emb: Optional[torch.Tensor] = None):
        ...
        if self.emb_layers is not None:
            ...
            if len(emb.shape) == 3:
                ...
                emb = rearrange(emb, "b f a c -> b c f a 1")
...
class ResnetBlock_raw(nn.Module):
    def __init__(
        self,
        dim: int,
        dim_out: int,
