...
class Diffusion(nn.Module):
    ...
    def add_shape_channels(self, x):
        if len(x.shape) <= 2:
            ...
            return rearrange(x, f"chunk b a -> chunk b 1 a {' 1' * (len(self.x_shape)-2)}")
    ...
    def compute_loss_weights(self, noise_levels: torch.Tensor):
        ...
        if len(cum_snr.shape) == 2:
            cum_snr = F.pad(cum_snr[:-1], (0, 0, 1, 0), value=0.0)
        else:
            cum_snr = F.pad(cum_snr[:-1], (0, 0, 0, 0, 1, 0), value=0.0)
    ...
    def forward(
        ...
        if len(noise_levels.shape) == 2:
            ...
            loss_weight = loss_weight.view(loss_weight.shape[0], loss_weight.shape[1], 1, loss_weight.shape[2], 1)
