...
class DiffusionForcingPlanning(DiffusionForcingBase):
    def __init__(
        ...
        self.n_tokens = self.episode_len // frame_stack + 1
        self.gamma = gamma
        self.open_loop_horizon = open_loop_horizon
        self.padding_mode = padding_mode
        self.plot_end_points = plot_start_goal and self.guidance_scale != 0
    ...
    def _preprocess_token(self, feature_tokens, frame_index):

        bundles = rearrange(feature_tokens, "b seq ... -> seq b ...")
        # masks = torch.ones_like(bundles[:,:,0])
        masks = ~(frame_index == 99 )
        masks = rearrange(masks, "b seq -> seq b")
        conditions = None

        # if not self.training:
        #     feature_tokens = feature_tokens.cpu()
        #     frame_index = frame_index.cpu()
        #     index = frame_index>=4
        #     noise = torch.randn(feature_tokens.shape)
        #     idx = index.unsqueeze(2).expand_as(feature_tokens)
        #     feature_tokens[idx] = noise[idx]
        #     feature_tokens = feature_tokens.to("cuda")
        #     bundles = rearrange(feature_tokens, "b seq ... -> seq b ...")
    ...
    def forward(self, feature_tokens, frame_index):
        xs, conditions, masks = self._preprocess_token(feature_tokens, frame_index)
        n_tokens, batch_size, fea_len = xs.shape
        weights = masks.float()
        ...
        #     random_terminal = rearrange(random_terminal, "b t -> t b")
        ...
        # print(xs_pred.shape, feature_tokens.shape)
        # input()
        
        output_dict = {
            "loss": loss,
            "xs_pred": xs_pred.permute(1, 0, 2),
            "xs": xs.permute(1, 0, 2),
        }

        return output_dict

    def generate_scheduling_matrix_old(self, horizon, frame_index, num_conditioned_frames=0):
        frame_index = np.array(frame_index.cpu())

        frame_index = frame_index - num_conditioned_frames
        frame_index[frame_index<0] = 0
        frame_index[frame_index>20] = -1
        
        first_column = frame_index[:, 0]
        frame_index = frame_index - first_column[:, np.newaxis]
        frame_index_seq = []

        seq_length = 20
        for i in range(frame_index.shape[0]):
            frame = frame_index[i]
            # frame = frame[frame >= 0] 
            frame = frame/seq_length*self.sampling_timesteps
            frame[frame < 0] = self.sampling_timesteps
            frame = np.ceil(frame).astype(int)
            frame_index_seq.append(frame)
        frame_index_seq = np.vstack(frame_index_seq)

        frame_index_matrix = []

        while not np.all(frame_index_seq == 0):
            frame_index_matrix.append(frame_index_seq.copy())
            frame_index_seq -= 1
            frame_index_seq[frame_index_seq < 0] = 0
        frame_index_matrix.append(frame_index_seq)
        frame_index_matrix = rearrange(np.hstack(frame_index_matrix), "B (step seq) -> B step seq", seq=frame_index.shape[-1])
        return frame_index_matrix

    def generate_scheduling_matrix(self, horizon, frame_index, num_conditioned_frames=0, uncertainty_scale=0.7, frame_para=1):
        frame_index = np.array(frame_index.cpu())

        frame_index = frame_index - num_conditioned_frames
        frame_index[frame_index<0] = 0
        # frame_index[frame_index>20] = -1
        frame_index[frame_index>20] = 20
        ...
        m = 0
        frame_id = 1
        tmp_frame_index = frame_index_seq.copy()
        tmp_frame_index = np.full(frame_index_seq.shape, self.sampling_timesteps)
        tmp_frame_index[frame_index_seq==0] = 0

        while not np.all(tmp_frame_index == 0):
            index = frame_index_seq<frame_id
            tmp_frame_index[index] = tmp_frame_index[index] -1
            if np.max(tmp_frame_index[index]) <= uncertainty_scale*self.sampling_timesteps:
                frame_id += frame_para
            tmp_frame_index[tmp_frame_index<0] = 0
            frame_index_matrix.append(tmp_frame_index.copy())
        # frame_index_matrix.append(frame_index_seq)
        frame_index_matrix = rearrange(np.hstack(frame_index_matrix), "B (step seq) -> B step seq", seq=frame_index.shape[-1])
        return frame_index_matrix
    ...
    @torch.no_grad()
    def inference(self, feature_tokens, frame_index, num_conditioned_frames, namespace="inference"):
        xs, conditions, masks = self._preprocess_token(feature_tokens, frame_index)
        n_tokens, batch_size, fea_len = xs.shape

        if self.guidance_scale == 0:
            namespace += "_no_guidance_"
        horizon = self.episode_len

        guidance_fn = None

        horizon = n_tokens
        scheduling_matrix = self.generate_scheduling_matrix(horizon, frame_index, num_conditioned_frames) # [B, steps, tokens]
        ...
        for m in range(scheduling_matrix.shape[1] - 1):
            ...
            from_noise_levels = torch.from_numpy(from_noise_levels)
            to_noise_levels = torch.from_numpy(to_noise_levels)
            from_noise_levels = rearrange(from_noise_levels, "B seq -> seq B")
            to_noise_levels = rearrange(to_noise_levels, "B seq -> seq B")

            plan = self.diffusion_model.sample_step(
                plan, conditions, from_noise_levels, to_noise_levels, guidance_fn=guidance_fn
            )
            plan_hist.append(plan.detach())

        plan_hist = torch.stack(plan_hist) # [m seq b c]
        # plan_hist = rearrange(plan_hist, "m seq b (fs c) -> m (seq fs) b c", fs=self.frame_stack)

        return plan_hist[-1].permute(1, 0, 2)
