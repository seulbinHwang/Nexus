...
class GUMPTokenDecoder(nn.Module):
    ...
    def decoding_agents_single_frame(self, embedder, output_features, tokenized_arrays, last_frame_only=False) -> Dict:
        """
        Dummy forward
        """

        query_embedding_inds, state_embedding_features, return_list = gutils.get_agent_target_inds(tokenized_arrays, self.block_size, last_frame_only=last_frame_only, return_list=True)
        query_embedding_inds = torch.tensor(query_embedding_inds, dtype=torch.long).to(output_features.device)
        target_tokenized_state = torch.tensor(state_embedding_features, dtype=torch.long).to(output_features.device)
        ...
        # assert hidden.shape[1] == output_features.shape[0]
        return pred_agent_logits, pred_agent_tokens, target_tokenized_state, hidden, return_list
